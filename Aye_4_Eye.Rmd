...
Here, thanks to Dr. Vica Papp is a script to downsample a historical linguistic dataset and grow a random forest on this data. For this example I am using a dataset of 18th century Scots and English words present in a corpus of texts produced by politically-active authors. There is far more English (92%) present in the dataset than Scots (8%). As it is the Scots I am interested in, a successful statistical analysis requires downsampling. 
...
Make sure dataset is filtered by desired predictor (in this case dataset only contains authors with known pro/anti-Union sentiments). Read in original dataset (datBigrf1), create a new copy containing only these authors (datBigrf2):
...
Select predictors to be included in analysis, treat Year of Birth as Numeric, the rest as Factor:
...
A five-fold cross-validated forest on 200 trees, which took about 10 minutes to grow. The accuracy, unsurprisingly is about 92%, and Cohen's Kappa, which gives you the classification accuracy normalized by the imbalance of the two classes in the data, is not too shiny either.
...
- a hybrid of under and oversampling (with the ROSE or SMOTE algorithms): these are costly to compute so cache them if you run them
